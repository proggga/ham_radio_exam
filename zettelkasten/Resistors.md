---
id: 202301011212
title: "Resistors"
tags: ["ham-radio", "components", "index"]
created: 2025-12-29
type: index
modified: 2025-12-29

dutch_title: "Weerstanden"
aliases: ["Weerstanden"]
---

# Resistors

This map covers resistor types, identification, and standard values.

## Hardware
*   **[[Resistor Types]]** - Fixed, Variable, and Thermistors.

## Identification
*   **[[Resistor Color Code]]** - Decoding 4-band resistors.
*   **[[Standard Component Values (E-Series)]]** - E12 and E24 series.

## 4. Resistance of a Wire
*   **Formula:** $R = \rho \frac{L}{A}$
    *   $\rho$ (Greek: Rho) = **[[Resistivity (Soortelijke Weerstand)|Resistivity]]**.
*   **Deformation (Stretching):** If a wire is stretched to **n** times its length, its volume remains constant ($V = L \times A$). Therefore, the Area decreases by **n** times.
    *   New Length = $n \cdot L$
    *   New Area = $A / n$
    *   New Resistance = $R_{new} = \rho \frac{n \cdot L}{A / n} = n^2 \cdot R$.
    *   *Example:* Stretching a wire to 2x length results in **4x** resistance.

## Related
*   **[[Voltage, Current, and Ohm's Law]]** - The fundamental law governing resistors.
