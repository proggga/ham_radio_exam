...
*   **Linearity:**
    *   **SSB/AM:** Requires **Linear** amplification ([[Amplifiers|Class A or AB]]) to preserve the envelope shape. Non-linearity causes **Splatter** (IMD).
    *   **CW/FM:** Can use **Non-linear** amplification ([[Amplifiers|Class C]]) for higher efficiency, as amplitude contains no information.

## 2. Output Filters
Power amplifiers generate harmonics (multiples of the frequency).
*   **Low Pass Filter (LPF):** Essential at the output to suppress harmonics to legal levels. See [[Filters & Resonance|Filters]].
...
*   **Note:** An ATU at the transmitter does **NOT** fix the [[Matching and SWR|SWR]] on the feedline. It only matches the *input* of the feedline to the TX.
