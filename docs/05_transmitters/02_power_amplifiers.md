...
*   **Low Pass Filter (LPF):** Essential at the output to suppress harmonics to legal levels.
*   **Pi-Filter:** A common LC circuit ($\pi$-shape) used for both impedance matching and low-pass filtering.

## 3. Antenna Matching (ATU)
Matches the transmitter impedance (usually $50 \Omega$) to the antenna system.
