...
*   **SINAD:** Signal-to-[Noise](../01_electricity/14_AC_Signals_&_Noise.md) and Distortion ratio. Typical spec: 12dB SINAD.
*   **[Noise](../01_electricity/14_AC_Signals_&_Noise.md) Figure (Ruisgetal, F):** Measure of how much noise the receiver adds.
    $$F = \frac{(S/N)_{in}}{(S/N)_{out}}$$
    *   Ideal receiver: $F = 1$ (0 dB).
    *   **Independence:** [Noise](../01_electricity/14_AC_Signals_&_Noise.md) Figure is **independent** of bandwidth.
    *   **LNA (Low Noise Amplifier):** The first stage (RF Amp) is critical for system noise figure (Friis formula). See [Amplifiers](../03_circuits/06_Amplifiers.md).

## 2. Selectivity (Selectiviteit)
The ability to separate the wanted signal from others.
*   **Bandwidth:** Determined by IF filters (Crystal, Mechanical, Ceramic). See [Filters](../03_circuits/03_Filters_&_Resonance.md).
