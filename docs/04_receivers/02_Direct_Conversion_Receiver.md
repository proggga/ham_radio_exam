...
*   **Intermediate Frequency (MF)**: Since $f_{RF} \approx f_{LO}$, the resulting difference frequency is $0 \text{ Hz}$ (or audio frequencies). There is **no** IF amplifier stage like in a [Superheterodyne Receiver](03_Superheterodyne_Receiver.md).
...
## Characteristics
*   **Modes**:
    *   Excellent for **[SSB](../01_electricity/35_Single_Sideband_SSB.md)** and **[CW](../01_electricity/34_CW_Abbreviations_&_Prosigns.md)**.
    *   Can receive **[AM](../01_electricity/33_Analogue_Modulation_&_AM.md)**, but requires very precise tuning to achieve "Zero Beat" (where $f_{LO} = f_{Carrier}$). If slightly off, a loud audible tone (heterodyne) is heard.
