...
*   **BFO Setting**:
    *   **[CW](../01_electricity/34_CW_Abbreviations_&_Prosigns.md)**: Set BFO $\approx 800$ Hz away from IF center to hear a tone.
...
*   **Limiter (Begrenzer)**: Essential before detection to remove Amplitude variations ([Noise](../01_electricity/27_AC_Signals_&_Noise.md)/Static). Usually a saturated amplifier.
...
*   **Ratio Detector**: [AM](../01_electricity/33_Analogue_Modulation_&_AM.md)-insensitive (built-in limiting action via capacitor).
*   **Quadrature Detector**: Uses a phase-shift network and product detector. Common in ICs.
*   **[PLL](../03_circuits/23_Phase_Locked_Loop_PLL.md) Detector**: A [Phase Locked Loop](../03_circuits/23_Phase_Locked_Loop_PLL.md) locks to the input signal. The **Error Voltage** that controls the VCO tracks the frequency deviation $\rightarrow$ This is the Audio output. High quality.
