...
*   **Short Circuit Current ($I_{short}$):** Maximum current if terminals are shorted ($R_{load} = 0$).
    *   $I_{short} = \frac{E}{R_i}$ (from [Ohm's Law](07_Voltage,_Current,_and_Ohm's_Law.md))
...
*   **[Transistor](../02_components/18_Transistors_BJT_&_FET.md) Current Source:** A bipolar transistor with a fixed base voltage and emitter resistor acts as a **Constant Current Source**.
