...
*   **DAC (Digital-to-Analog Converter)**: Converts the stream of numbers back into a stepped voltage.
...
*   **Quantization [Noise](14_AC_Signals_&_Noise.md)**: The error between the real analog value and the nearest digital step.
    *   More bits = Less noise, higher [Dynamic Range](../04_receivers/05_Receiver_Performance.md). (~6 dB per bit).

## Digital Filters
*   **Convolution**: The mathematical operation used to apply filters ([Convolution](22_Convolution.md)).
...
*   **[Direct Digital Synthesis (DDS)](../03_circuits/08_Direct_Digital_Synthesis_DDS.md)**: Generating waveforms directly from a digital look-up table (Sine table) and a DAC.
