...
*   **Hartley**: Uses an **inductive** voltage divider (tapped coil).
    *   *Mnemonic:* **H**artley = **H**enry ([Inductor](../02_components/09_Inductors.md)).
...
*   **Component**: Uses a **[Varicap](../02_components/22_Varicap.md)** (capacitance diode) in the tuned circuit. Reverse voltage changes capacitance $\to$ changes frequency.

## Stability & Phase Noise
*   **Phase [Noise](../01_electricity/27_AC_Signals_&_Noise.md)**: Short-term frequency instability (jitter). Appears as noise sidebands. Widens the signal and degrades receiver selectivity.
