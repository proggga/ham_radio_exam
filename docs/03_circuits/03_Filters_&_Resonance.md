...
*   **[Quality Factor (Q)](06_Quality_Factor_Q.md)** - Q factor.
    *   *Caveat:* Adding a parallel resistor (damping) to an [LC circuit](01_Reactive_Combinations.md) **lowers** the [Q-factor](06_Quality_Factor_Q.md), **increases** the [Bandwidth](07_Bandwidth.md), and **lowers** the impedance/gain.
*   **[Bandwidth](07_Bandwidth.md)** - -3dB bandwidth and selectivity.
## 3. Bandwidth and Selectivity
*   **[Bandwidth](07_Bandwidth.md) ($B$):** $B = \frac{f_{res}}{Q}$.
*   **Parallel RLC:**
    *   Lower $R$ (more damping? No, in parallel, Lower R = More Load = Lower Q).
    *   Wait, parallel resistor acts as load. $Q = R_p / X$.
    *   If $R_p$ decreases (more loading), $Q$ decreases, [Bandwidth](07_Bandwidth.md) **Increases**.
...
*   **Series LC:** $X_L$ and $X_C$ cancel out ($X_{total} = 0$). [Impedance](../01_electricity/22_Impedance.md) is minimal ($Z = R_{loss}$). Acts as a short circuit.
...
*   **Parallel LC:** Currents cancel out. [Impedance](../01_electricity/22_Impedance.md) is maximal ($Z_{res} = L / (C \cdot R_{loss})$ or $Q \cdot X_L$). Acts as an open circuit.
    *   **Off-[Resonance](05_Resonance.md) Behavior:**
...
When two tuned circuits are coupled (e.g., in an IF transformer), the response changes based on the coupling coefficient ($k$) and [Q-factor](06_Quality_Factor_Q.md) ($Q$).
...
*   **Overcritical Coupling ($kQ > 1$):** Double peak ("Camel hump" or "Oortjes"). Broader bandwidth but with a dip in the center. Used to widen the passband for [FM](../01_electricity/36_Frequency_Modulation_FM.md)/Audio.
