...
*   **[Sensitivity](../04_receivers/04_Receiver_Performance.md) ($S$):** Expressed in $k\Omega/V$ (e.g., $20 k\Omega/V$). $S = 1 / I_{fsd}$.
...
*   **Loading Effect (Belastingseffect):**
    ...
    *   If $R_{in}$ is not $\gg$ Circuit [Impedance](../01_electricity/22_Impedance.md), the meter loads the circuit, reading **lower** than the actual voltage.
    *   *[Exam](../12_regulations/05_Licensing_in_the_Netherlands.md) Tip:* Always check if the meter resistance is comparable to the circuit resistors. If so, calculate the parallel equivalent.
...
Uses an Analog-to-Digital Converter ([ADC](../01_electricity/41_Digital_Processing_Techniques.md)).
*   **Input [Impedance](../01_electricity/22_Impedance.md):** Very high and constant (typically $10-11 M\Omega$). Does not load the circuit significantly.
