...
*   **Voltage Divider Method (Stable):**
    ...
    5.  Collector Voltage ($U_c$) set by Collector [Resistor](01_Resistors.md) ($R_c$) and supply ($U_{supply}$): $U_c = U_{supply} - I_c \cdot R_c$.
*   **Gain Formula (Approx):**
...
*   **Input [Impedance](../01_electricity/22_Impedance.md):** Extremely high (Gate draws almost 0 current).
*   **J-FET (Junction FET):**
...
*   **MOSFET (Metal-Oxide-[Semiconductor](15_Semiconductors.md) FET):**
    ...
    *   **Dual-Gate MOSFET:** Two gates. Gate 2 often used for Gain Control ([AGC](../04_receivers/05_Automatic_Gain_Control_AGC.md)) or isolating input from output (reduced Miller effect).
    *   **Handling (ESD [Safety](../10_safety/01_Electrical_Safety.md))**: The thin oxide layer is easily punctured by Static Electricity (ESD). Always handle with grounded tools/wrist straps and keep leads shorted until soldering.

### FET Amplification
*   **Voltage Gain ($A_u$):**
    *   With Source [Resistor](01_Resistors.md) ($R_S$) unbypassed: $A_u \approx \frac{R_D}{R_S}$.
...
| **Input [Impedance](../01_electricity/22_Impedance.md)** | Low/Medium | Very High |
| **Charge Carriers** | Electrons & Holes (Bipolar) | Electrons (N) or Holes (P) (Unipolar) |
| **[Noise](../01_electricity/27_AC_Signals_&_Noise.md)** | Higher | Lower (generally) |
| **[Sensitivity](../04_receivers/04_Receiver_Performance.md)** | Robust | Sensitive to static (MOSFET) |
