...
*   **[Impedance](../01_electricity/22_Impedance.md)**: Very high input impedance ($Z_{in}$), low output impedance ($Z_{out}$).
...
*   **Input [Impedance](../01_electricity/22_Impedance.md)**: Equal to $R_1$ (Virtual ground at input).
*   **Virtual Ground**: The (-) input is at 0V potential but not connected to ground.
*   *[Exam](../12_regulations/05_Licensing_in_the_Netherlands.md) Tip:* Pay attention to the **minus sign** in the formula. It indicates the output is **in anti-phase** ($180^\circ$ shifted) relative to the input.

### 2. Non-Inverting Amplifier
Output is in phase with input.
*   **Gain**: $A_u = 1 + \frac{R_2}{R_1}$ (where $R_2$ is feedback, $R_1$ is to ground).
*   **Input [Impedance](../01_electricity/22_Impedance.md)**: Extremely High (Input connects directly to Op-Amp gate/base).
